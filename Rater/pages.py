...
class Task(Page):
    ...
    def before_next_page(self):
        ...
        if self.participant.vars['count_round'] == 24 or len(list) == 0:
            self.participant.vars['list_is_empty'] = 1
        else:
            self.participant.vars['list_is_empty'] = 0

        # print("if 1, list is empty", self.participant.vars['list_is_empty'])
        # print("count", self.participant.vars['count_round'])
        # print("count", self.session.vars['count_obs'])
        # print("obs count alle", self.session.vars['count_obs'][self.player.x])
